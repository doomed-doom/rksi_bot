...
class Student(DBConfig):
    def check_user_session(self, session_id):
        self.mycursor.execute(f'SELECT * FROM halaton.users WHERE session_id = {session_id}')
        result = self.mycursor.fetchall()
        return result

    async def leave_user_session(self, session_id):  # изменяет сессию юзера в бд
        self.mycursor.execute(f"UPDATE halaton.users SET session_id = 0 WHERE session_id = {session_id};")
        self.db.commit()

    async def delete_user_account(self, login, password, main_group, session_id):  # изменяет сессию юзера в бд
        self.mycursor.execute(
            f"SELECT * FROM halaton.users  WHERE login = '{login}' and password = '{password}' and session_id = {session_id} and main_group = '{main_group}'")
        result = self.mycursor.fetchall()
        if result:
            self.mycursor.execute(
                f"DELETE FROM halaton.users  WHERE login = '{login}' and password = '{password}' and session_id = {session_id} and main_group = '{main_group}'")
            self.db.commit()
            return result
        else:
            return result
...
class Schedule(DBConfig):
    def get_group_count(self):
        self.mycursor.execute("SELECT DISTINCT group_name FROM halaton.schedule")
        result = self.mycursor.fetchall()
        return result

    def get_teacher_count(self):
        self.mycursor.execute("SELECT DISTINCT teacher FROM halaton.schedule")
        result = self.mycursor.fetchall()
        return result

    def get_schedule_teacher(self, teacher_name, day):
        self.mycursor.execute(f'SELECT * FROM halaton.schedule WHERE teacher = "{teacher_name}" AND day = "{day}"')
        result = self.mycursor.fetchall()
        return result

    def get_schedule_group(self, group_name, day):
        self.mycursor.execute(f'SELECT * FROM halaton.schedule WHERE group_name = "{group_name}" AND day = "{day}"')
        result = self.mycursor.fetchall()
        return result
...
class Logging(DBConfig):
    async def check_user(self, login, password):
        self.mycursor.execute(f'SELECT * FROM halaton.users WHERE login = "{login}" AND password = "{password}"')
        result = self.mycursor.fetchall()
        return result

    async def update_user_session(self, session_id, login):  # изменяет сессию юзера в бд
        self.mycursor.execute(f"UPDATE halaton.users SET session_id = {int(session_id)} WHERE login = '{login}';")
        self.db.commit()
...
class StartUser(DBConfig):
    async def add_user(self, user_id, role='member', username='None'):
        self.mycursor.execute(
            f"INSERT INTO halaton.user (user_id, role, username) VALUES ({user_id}, '{role}', '{username}');")
        self.db.commit()

    async def find_user(self, username):
        self.mycursor.execute(f'SELECT * FROM halaton.user WHERE username = "{username}";')
        result = self.mycursor.fetchall()
        if result:
            return result

    async def check_user(self, user_id, username='None'):
        self.mycursor.execute(f'SELECT * FROM halaton.user WHERE user_id = {user_id};')
        if not self.mycursor.fetchall():
            await self.add_user(user_id, username=username)
        else:
            self.mycursor.execute(
                f"SELECT * FROM halaton.user WHERE role = 'owner' AND user_id = {user_id} OR role = 'admin' AND user_id = {user_id};")
            result = self.mycursor.fetchall()
            if result:
                return result

    async def change_role(self, username, role):
        self.mycursor.execute(f"UPDATE halaton.user SET role = '{role}' WHERE username = '{username}';")
        self.db.commit()

    def get_accounts_count(self):
        self.mycursor.execute("SELECT login FROM halaton.users;")
        result = self.mycursor.fetchall()
        return result

    async def del_account(self, login):
        self.mycursor.execute(
            f"DELETE FROM halaton.users WHERE login = '{login}';")
        self.db.commit()

    async def change_attribute(self, method, old_value, new_value):
        self.mycursor.execute(f"UPDATE halaton.users SET {method} = '{new_value}' WHERE login = '{old_value}';")
        self.db.commit()
...
class SpamUser(DBConfig):
    async def spam_all(self):
        self.mycursor.execute(
            f"SELECT user_id FROM halaton.user;")
        result = self.mycursor.fetchall()
        return result

    async def spam_group(self, group_name):
        self.mycursor.execute(
            f"SELECT session_id, main_group  FROM halaton.users WHERE main_group = '{group_name}';")
        result = self.mycursor.fetchall()
        return result

    async def spam_user_all(self):
        self.mycursor.execute(
            f"SELECT session_id  FROM halaton.users;")
        result = self.mycursor.fetchall()
        return result
...
class Files(DBConfig):
    async def add_file(self, file_id, file_name):
        self.mycursor.execute(
            f"INSERT INTO halaton.documents (file_id, file_name) VALUES ('{file_id}', '{file_name}')")
        self.db.commit()

    async def delete_file(self, file_name):
        self.mycursor.execute(
            f"DELETE FROM halaton.documents WHERE file_name = '{file_name}';")
        self.db.commit()

    def get_file_name(self):
        self.mycursor.execute("SELECT file_name FROM halaton.documents")
        result = self.mycursor.fetchall()
        return result

    async def take_file(self, file_name):
        self.mycursor.execute(f"SELECT * FROM halaton.documents WHERE file_name = '{file_name}'")
        result = self.mycursor.fetchall()
        return result
